package com.litongjava.openai.token;
...
public class OpenAiTokenizer implements Tokenizer {
  ...
  public int estimateTokenCountInText(String text) {
    return encoding.orElseThrow(unknownModelException()).countTokensOrdinary(text);
  }
  ...
  public List<Integer> encode(String text) {
    return encoding.orElseThrow(unknownModelException()).encodeOrdinary(text).boxed();
  }

  public List<Integer> encode(String text, int maxTokensToEncode) {
    return encoding.orElseThrow(unknownModelException()).encodeOrdinary(text, maxTokensToEncode).getTokens().boxed();
  }
  ...
  public String decode(List<Integer> tokens) {
    ...
    return encoding.orElseThrow(unknownModelException()).decode(intArrayList);
  }
  ...
  public int estimateTokenCountInToolExecutionRequests(Iterable<ToolExecutionRequest> toolExecutionRequests) {
    ...
    if (modelName.equals(OpenAiModels.GPT_3_5_TURBO_1106) || isOneOfLatestGpt4Models()) {
      tokenCount += 16;
    ...
    if (modelName.equals(OpenAiModels.GPT_4_1106_PREVIEW)) {
      tokenCount += 3;
      if (toolsCount > 1) {
        tokenCount += 18;
  ...
  public int estimateTokenCountInForcefulToolExecutionRequest(ToolExecutionRequest toolExecutionRequest) {
    ...
    if (modelName.equals(OpenAiModels.GPT_3_5_TURBO_1106)) {
      int argumentsCount = countArguments(toolExecutionRequest.arguments());
      if (argumentsCount == 0) {
        return 1;
      }
  ...
  private boolean isOneOfLatestGpt4Models() {
    return modelName.equals(OpenAiModels.GPT_4_TURBO_PREVIEW) || modelName.equals(OpenAiModels.GPT_4_1106_PREVIEW) || modelName.equals(OpenAiModels.GPT_4_0125_PREVIEW);
  }
}
